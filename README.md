...
The type you should primarily use with this library is `Fixed`. `Fixed64` is only used internally for multiplication and division.
...
[^3]: You should probably use this with common sense. Floats and fixed point don't go well together.<br>For paranoid people, it's more reliable to generate constant fixed point numbers from floats by typing `x * 65536` in a calculator and storing a truncated version of the result with a `Fixed` cast.
